...
public class DBModel {
    ...
    Логер для DBModel
     */
    private static final Logger logger = Logger.getLogger(Model.DBModel.class);
     ...
     */
    public DBModel() throws ClassNotFoundException {
        initDB();
    }
    ...
    private void initDB() throws ClassNotFoundException {
        ...
        try {
            ...
            /*
            connectionFlag для відображення повідомлення в AlertMessage
             */
            connectionFlag = true;
    ...
    public void connAlert() {
        ...
        if (connectionFlag) {
            alertMessage.showAlert("Підключеня активне!","Все добре!",2);

        } else {
            alertMessage.showAlert("Помилка підключення до БД!","Помилка?",0);
        }
    }
    ...
    /*
    Копіювання колекції з MainController в DBModel для подальшої обробки даних.
